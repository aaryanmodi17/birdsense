...
```
You also need the raw eBird data (see Data below) placed at:
```
...
You need a Google account with Earth Engine access and a Google Cloud project
...
Authenticate once — opens a browser, stores a token under `~/.config/earthengine/`:
```bash
...
```bash
   ...
   ```
Verify with the five-point eyeball test, which prints raw and converted values
...
Habitat guild assignment for H3 — particularly Bar-headed Goose (uses both
lakes and grasslands) and Greylag Goose (wetland-associated but grazes farmland).
The N=2 confirmation threshold, inherited as a working default and never
reviewed against published phenology methodology.
The Nov–Feb winter window, applied uniformly and not verified against each
species' actual residence period.
The ≥20-checklist site-level sample-size threshold, identified post hoc from
