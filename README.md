...
- **The vanishing advance.** Arrival date is inversely correlated with observer effort
  ...
  January 1 by 2021.
- **The disappearing decline.** Northern Pintail's statewide detection rate falls
  0.19 → 0.07, implying a collapse. At Nal Sarovar — a fixed, consistently-birded
  hotspot — it is essentially flat. The statewide "decline" is denominator inflation
  as eBird spread into habitat where Pintail were never likely.
- **What survives.** Bar-headed Goose detection rate at Thol Lake rose ~82%
  (p ≈ 0.01) under the same effort correction — a candidate real signal, consistent
  with documented regional expansion.
...
| 12 | Dalmatian Pelican | *Pelecanus crispus* | wetland | see note |
...
> that: it is well represented and confirmed in all 16 years, and it behaves like
> the other eleven (naive arrival slope −0.674 days/yr, p = 0.0009; effort-robust
> peak week not significant). Its "earlier arrival" is the same artifact.
> *Note: the record count is reported as 12,444 in `paper/manuscript.md` but 10,000
> in `paper/tables/tableS4_species.csv` — unreconciled, do not quote either without
> checking.*
...
The paper carries its own six figures and four supporting tables in `paper/`.
